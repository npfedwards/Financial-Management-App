			...
			if($rt!=NULL && $rf!=NULL && $rt!="undefined" && $rf!="undefined"){
				if($rf=='m'){ //If it's monthly we do it based on day of the month
					...
					if($insertid!=0){ //Repeat Paired Payment
						$query="INSERT INTO repeats (Frequency, Times, ExpireTime, UserID, AccountID, Timestamp, PaymentName, PaymentDesc, PaymentAmount, PaymentType, ToAccount) VALUES ('$rf', '$rt', '$expiretime', '$user', '$toaccount', '$time', '$otherparty', '$desc', '$amount', '$type', '$account')";
